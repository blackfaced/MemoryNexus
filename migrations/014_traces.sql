-- Minimal Trace evidence layer.
-- Trace is scoped to a Cognitive Space; Namespace remains an optional domain
-- partition inside that Space, not a permission boundary.
...
CREATE TABLE IF NOT EXISTS traces (
    ...
    CONSTRAINT traces_namespace_same_space_fkey
        ...
        ON DELETE SET NULL (namespace_id),
    ...
    CONSTRAINT traces_task_type_check
        CHECK (task_type IN ('chat', 'search', 'lens_run', 'review', 'practice', 'feedback', 'planning', 'install', 'profile', 'routing', 'consolidation', 'dreaming')),
    CONSTRAINT traces_mode_check
        CHECK (mode IN ('fast', 'focused', 'deep', 'none')),
    ...
    CONSTRAINT traces_completed_status_check
        CHECK (status <> 'completed' OR completed_at IS NOT NULL),
    CONSTRAINT traces_latency_non_negative_check
        CHECK (latency_ms IS NULL OR latency_ms >= 0),
    CONSTRAINT traces_cost_non_negative_check
        CHECK (estimated_cost_usd IS NULL OR estimated_cost_usd >= 0),
    CONSTRAINT traces_local_processing_ratio_check
        CHECK (local_processing_ratio IS NULL OR (local_processing_ratio >= 0 AND local_processing_ratio <= 1))
);

CREATE INDEX IF NOT EXISTS idx_traces_space_id
    ON traces(space_id);

CREATE INDEX IF NOT EXISTS idx_traces_namespace_id
    ON traces(namespace_id);

CREATE INDEX IF NOT EXISTS idx_traces_task_type
    ON traces(task_type);

CREATE INDEX IF NOT EXISTS idx_traces_mode
    ON traces(mode);

CREATE INDEX IF NOT EXISTS idx_traces_runtime
    ON traces(runtime);
...
CREATE INDEX IF NOT EXISTS idx_traces_space_started_at
    ON traces(space_id, started_at DESC);
