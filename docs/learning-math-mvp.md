...
`learning.math` is the first product MVP candidate for MemoryNexus. Thought
Review remains the reflective demo and presentation entry point; `learning.math`
tests whether the same Space-owned memory foundation can become a concrete
long-term feedback engine for skill practice.

The first scenario is parent-assisted elementary math mistake feedback for
fraction word problems. The product should help a parent and child see what was
practiced, what answer was given, what mistake pattern appeared, what feedback
was useful, and what next exercise should come after it.

## Product Boundary

`CognitiveSpace` remains the ownership and permission boundary. Parents and
children work inside a Space, and every Namespace, FeedbackLoop, practice
Memory, report, and later projection must be validated through Space membership.

`Namespace` is only a domain partition inside the Space. `learning.math` means
the practice belongs to the elementary math skill domain; it does not introduce
separate membership, authorization, or data ownership.

The MVP should use parent and child language in the product surface:
...
responses aimed at a parent-assisted flow.
...
or reuse the `learning.math` namespace, enter the practice goal and task, record
the child's answer or reasoning, review the evaluation, and use the weekly
...
not require a separate autonomous child account.

### Child

The child solves the fraction word problems and may explain their reasoning. The
system records the child's answer and reasoning as practice evidence, but the
child is not expected to manage the product, configure goals, or interact with a
full tutoring chatbot.

## First Scenario

Fraction word problems are the only first scenario.
...
Attempt: answers and rough reasoning submitted by the child
...
2. Parent opens the `learning.math` practice area.
3. The app creates or reuses the `learning.math` Namespace inside that Space.
4. Parent creates a practice session with a learning goal and fraction word
   problem task.
5. Child completes the task outside or inside the UI.
6. Parent records the child's answer and reasoning.
...
fields into concrete math practice language:

| FeedbackLoop field | Math practice language | Example |
| --- | --- | --- |
...
| `attempt` | child answer / reasoning | Answers, scratch reasoning, or parent-entered summary |
...
Primary labels should be parent and child friendly: practice, answer, mistake
...
be generated for a `learning.math` Namespace and date window inside one
`CognitiveSpace`.
...
- Do not require autonomous child accounts in the first MVP.
...
The MVP PRD is satisfied when the learning product remains narrowly scoped to a
parent-assisted fraction word problem feedback loop, maps FeedbackLoop fields to
math practice language, keeps practice events as Memory, defines the API/MCP/UI
slice dependencies, and specifies a weekly learning review that is traceable to
Space-owned practice sources.
