...
PracticePlan
  |-- is the product-facing selected next action
  |-- may be sourced from a DreamCandidate selected by a Wake path
  |-- may also be sourced directly from a ConsolidationResult for manual plans
  |-- records selected, executed, and evaluated states
  `-- is evaluated by later Trace / FeedbackLoop evidence
```
...
`PracticePlan` is the product-facing next action selected from Sleep or
Planning evidence. It should be safe for Surfaces or Adapters to show as the
next practice, review, or planning step, while `DreamCandidate` remains an
internal candidate object.

PracticePlan answers:

```text
Which next action was selected for the user-facing practice loop, and did it
help after execution?
```
...
| `id` | yes | Stable UUID. |
| `space_id` | yes | Owning `CognitiveSpace`; all permissions remain Space-based. |
| `namespace_id` | no | Domain partition; usually matches the source candidate or consolidation result. |
| `source` | yes | Either `dream_candidate` or `consolidation_result`. |
| `purpose` | yes | Product-facing reason for the plan, such as tomorrow dictation practice or weekly review. |
| `content` | yes | The selected plan text or practice instruction. |
| `expected_effect` | no | What the plan is expected to improve, test, or reveal. |
| `status` | yes | `selected`, `executed`, `evaluated`, or `cancelled`. |
| `selected_at` | no | Timestamp when the plan was selected for use. |
| `executed_trace_ids` | no | Traces from Wake interactions that executed this plan. |
| `evaluation_trace_ids` | no | Later traces used to judge whether it helped. |
| `evaluation_result` | no | Best-effort summary such as useful, neutral, harmful, or inconclusive. |
| `created_at` | yes | Timestamp when the plan was created. |
...
- `dream_candidate`: the plan was selected from a DreamCandidate generated by
  Dreaming.
- `consolidation_result`: the plan was created directly from consolidated
  evidence, usually for manual or deterministic Planning paths where no
  candidate object was generated.
...
The first domain implementation records the lifecycle only. It does not
generate plans, persist them, expose them through an API, or call AI providers.
