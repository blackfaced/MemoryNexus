# 📖 MemoryNexus 文档

## 目录

| 文档 | 说明 |
|------|------|
| [🏗️ 架构设计](architecture.md) | 系统整体架构 |
| [🔌 API 文档](api.md) | REST API 接口规范 |
| [🚀 部署指南](deployment.md) | 各平台部署教程 |
| [🛠️ 开发指南](development.md) | 本地开发说明 |
| [📅 路线图](roadmap.md) | 开发计划和里程碑 |
| [💡 常见问题](faq.md) | FAQ |

---

## 快速导航

### 部署
- [Docker 部署](deployment.md#docker-部署)
- [火山引擎部署](deployment.md#火山引擎)
- [阿里云部署](deployment.md#阿里云)
- [AWS 部署](deployment.md#aws)

### 开发
- [环境准备](development.md#环境准备)
- [本地运行](development.md#本地运行)
- [测试](development.md#测试)
- [代码规范](development.md#代码规范)

---

## 在线文档

- [API 文档 (Swagger)](http://localhost:8000/docs) - 服务启动后可访问
- [架构图](../assets/architecture.png)
