...
fn default_summary_length() -> usize { 50 }
fn default_language() -> String { "zh".to_string() }
fn default_include_keywords() -> bool { true }
...
pub trait Summarizer: Send + Sync {
    ...
    ) -> impl std::future::Future<Output = Result<SummaryResult, SummaryError>> + Send;
    ...
    fn summarize_batch(
        ...
        contents: Vec<String>,
        options: &SummaryOptions,
    ) -> impl std::future::Future<Output = Result<Vec<SummaryResult>, SummaryError>> + Send;
}
...
fn extract_keywords_simple(text: &str) -> Vec<String> {
    ...
    let stop_words = vec![
        ...
        "the", "and", "or", "but", "if", "then", "of", "for", "in", "on", "to",
    ];
    
    let words: Vec<&str> = text.split(|c: char| !c.is_alphanumeric())
