...
ADR-018 将 MemoryNexus 的总体定位更新为 local-first, namespace-based
long-term feedback engine。ADR-020 将第一上游产品方向从 broad
`learning.stem` / fraction-word-problem slice 调整为 Dictation Coach。本文中的
STEM Learning Feedback 仍作为 prior learning slice 和 FeedbackLoop 验证背景保留，
但不再是下一阶段唯一或首要产品入口。
...
- 第一产品 MVP 候选转向 STEM Learning Feedback，以 `learning.stem` 表示 STEM 学习
  领域，并用分数应用题作为第一验证任务。
...
- learning.stem / STEM Learning Feedback 是否是更强落地入口
...
2. Skill Namespace
   - 示例：`learning.stem`、`music.piano`、`chess.tactics`、`art.drawing`
...
但它不再作为第一商业/产品 MVP 候选。本文曾将第一产品 MVP 候选调整为：

```text
learning.stem
```

STEM Learning Feedback 使用 `learning.stem` 作为产品 namespace。第一验证任务是
parent-assisted elementary fraction word problems feedback loop，用家长可理解的语言验证：

```text
知识点 → 练习题 → 作答 → 错因 → 反馈 → 下一题 → 每周学习报告
```

ADR-020 之后，这条线被视为 prior learning slice。新的第一上游产品方向是 Dictation
Coach，用中文默写和英语 spelling / sentence dictation 验证 Surface Gateway 与
Trace -> FeedbackLoop -> GrowthModel -> PracticePlan 闭环。
...
- `Lens Run`、`Review Report` 和 `CognitiveProfile` 可以追加
  ...
  `space_id` 做权限校验。
- STEM Learning Feedback 是 prior learning slice；新产品入口应按 ADR-020 的
  Dictation Coach 方向独立验证，不能和底层 schema foundation 混在一个交付里。
...
- STEM Learning Feedback 已经给 MemoryNexus 提供了更具体、可衡量的学习 slice；
  ADR-020 进一步将第一上游产品入口收敛到 Dictation Coach。
...
- 如果过早同时推进多个垂直产品，会稀释 STEM Learning Feedback MVP 的验证焦点。
